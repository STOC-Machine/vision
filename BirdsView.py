...
camera = False #Will be used to test camera loading
...
best_cam_rot_guess = 0
while (len(filenames) > 0 or camera): # Loop while there are more files or we're using the camera
    if (len(filenames) > 0): #If we're running purely on files
        filename = filenames.pop(0) #And get the first file in the list
        try: 
            img = cv2.imread(filename)
        except:
            print('Warning: cannot read image {}, skipping'.format(filename))
            continue
    ...
    cv2.circle(birds_view, (int(birds_view.shape[0]/2), int(birds_view.shape[1]/2)), 5, (255,255,0), -1)
    squares, best_cam_rot_guess = grid.getSquareStats(img, camera_matrix, np.array([[]]), best_cam_rot_guess) #It's a magic function! Yay!
    ...
    base_object_points = [[-square_length/2, -square_length/2, 0],
                         ...
                         [square_length/2, square_length/2, 0],
                         [square_length/2, -square_length/2, 0]]
    if(best_cam_rot_guess != 0):
        temp_cam_line = grid.vecadd(grid.scalarmult(best_cam_rot_guess,50), [birds_view.shape[0]/2, birds_view.shape[0]/2, 0])
        cv2.line(birds_view, (int(temp_cam_line[0]), int(temp_cam_line[1])), (int(birds_view.shape[0]/2), int(birds_view.shape[1]/2)),
                 (0,0,255), 1, cv2.LINE_AA)
    for square in squares:
        temp_vec = grid.vecsub(square.location, squares[0].location)

        for edge_index in range(4):
            temp_draw_vec = grid.vecadd(square.location, base_object_points[edge_index])
            temp_draw_vec2 = grid.vecadd(square.location, base_object_points[edge_index-1])
            cv2.line(birds_view, (int(temp_draw_vec[0]+birds_view.shape[0]/2), int(temp_draw_vec[1]+birds_view.shape[1]/2)),
                     (int(temp_draw_vec2[0]+birds_view.shape[0]/2), int(temp_draw_vec2[1]+birds_view.shape[1]/2)),
                     (255,255,255), 3, cv2.LINE_AA)

        temp_vec[0]=(square_length+square_gap)*round(temp_vec[0]/(square_length+square_gap),0)
        temp_vec[1]=(square_length+square_gap)*round(temp_vec[1]/(square_length+square_gap),0)
        temp_vec[2]=0

        for i in base_object_points:
            glued_square_coords.append([[grid.vecadd(i, temp_vec)[0]], [grid.vecadd(i,temp_vec)[1]], [grid.vecadd(i, temp_vec)[2]]])
    ...
    if (len(squares) > 0):
        ...
        inliers, full_r_vec, full_t_vec = cv2.solvePnP(glued_square_coords, glued_square_corners, camera_matrix,
                                                       distortion_coefficients, squares[0].rvec, squares[0].tvec, True) 
        #print full_t_vec,full_r_vec
        rot_matrix = cv2.Rodrigues(full_r_vec)
        camera_pos = np.multiply(cv2.transpose(rot_matrix[0]), -1).dot(full_t_vec)
        print(camera_pos)
        print(squares[0].location)

        cam_to_grid_transform = np.concatenate((cv2.transpose(rot_matrix[0]), camera_pos), axis=1)
        ...
        temp_cam_line2 = grid.vecadd(grid.scalarmult(cam_rot, 50), [birds_view.shape[0]/2, birds_view.shape[0]/2, 0])
        cv2.line(birds_view, (int(temp_cam_line2[0]), int(temp_cam_line2[1])), (int(birds_view.shape[0]/2), int(birds_view.shape[1]/2)),
    ...
    if(camera):
        if(len(squares) > 0):
            best_cam_rot_guess = squares[0].camRot
